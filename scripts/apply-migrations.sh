...
# If this database already has the Touchpoint schema (from a prior manual apply),
# record existing SQL files as applied so CREATE TYPE is not re-run.
if [ "$(psql -d touchpoint -Atqc "SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='people' AND column_name='department_id'")" = "1" ]; then
