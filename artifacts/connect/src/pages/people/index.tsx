...
import { Search, Filter, Mail, MapPin, Building2, User } from 'lucide-react';
import { cn } from '@/lib/utils';
import { PersonRole } from '@workspace/api-client-react/src/generated/api.schemas';
  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        ...
        <button className="px-4 py-2 bg-primary text-primary-foreground font-medium rounded-md shadow-sm hover:bg-primary/90 transition-colors">
          Add Person
        </button>
      </div>
      ...
      <div className="bg-card p-4 rounded-xl border border-border shadow-sm flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input 
            ...
            className="w-full pl-9 pr-4 py-2 bg-muted/50 border border-transparent rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all"
          />
        </div>
        <div className="flex items-center gap-2">
          <Filter className="h-4 w-4 text-muted-foreground" />
          <select 
            className="bg-muted/50 border border-transparent rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all"
            value={roleFilter || ''}
                  ...
                  <tr key={person.id} className="hover:bg-muted/30 transition-colors group">
                    ...
                    <td className="py-3 px-4 text-right">
                      <Link 
                        ...
                        className="opacity-0 group-hover:opacity-100 transition-opacity px-3 py-1.5 bg-secondary text-secondary-foreground text-xs font-medium rounded hover:bg-secondary/80"
                      >
                        View Profile
                      </Link>
                    </td>
