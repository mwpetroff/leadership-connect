      ...
      <div className="flex gap-2 mb-6 border-b border-border">
        <button 
          ...
          className={cn(
            ...
            filterUpcoming ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"
          )}
        >
          Upcoming Events
        </button>
            ...
            <Link key={event.id} href={`/events/${event.id}`}>
              <div className="group h-full bg-card border border-border rounded-xl shadow-sm overflow-hidden hover:shadow-md transition-all hover:border-primary/30 flex flex-col cursor-pointer">
                ...
                <div className="bg-muted/40 p-4 border-t border-border grid grid-cols-3 gap-2 text-center divide-x divide-border">
                  <div>
                    <div className="text-xl font-display font-bold text-foreground">{event.leaderCount || 0}</div>
                    <div className="text-xs text-muted-foreground mt-0.5">Leaders</div>
                  </div>
                  <div>
                    <div className="text-xl font-display font-bold text-foreground">{event.inviteeCount || 0}</div>
                    <div className="text-xs text-muted-foreground mt-0.5">Invited</div>
                  </div>
                  <div>
                    <div className="text-xl font-display font-bold text-foreground">{event.attendeeCount || 0}</div>
